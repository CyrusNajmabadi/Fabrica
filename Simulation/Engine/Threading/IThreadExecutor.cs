...
/// giving zero-overhead dispatch — no interface vtable indirection on the hot path.
///
/// <typeparamref name="TState"/> is the per-dispatch input data written by the
/// coordinator before signaling the worker (e.g. images for simulation, a render
/// frame for rendering).
/// </summary>
    ...
    where TState : struct
{
    ...
    /// park state.  <paramref name="state"/> is the input data set by the
