...
///
/// The runner receives the node, the sequence number, and a callback
/// (<paramref name="saveAction"/>) to invoke when the work runs.  The callback
/// encapsulates the unpin and reschedule logic so the runner implementation
/// only needs to arrange execution (e.g. Task.Run) without knowing save internals.
///
...
/// </summary>
internal interface ISaveRunner<TNode> where TNode : ChainNode<TNode>
{
