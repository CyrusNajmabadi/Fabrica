...
///
/// Each <see cref="SimulationWorker"/> owns a dedicated WorkerResources
/// instance, giving it exclusive access to pools and creation-tracking
/// lists during tick computation — no locking required.
///
/// FUTURE CONTENTS
...
///
/// TICK LIFECYCLE
///   Before each tick dispatch, <see cref="PrepareForTick"/> clears any
///   per-tick accumulation state (e.g. the created-nodes list) so each
///   tick begins with a clean slate.
/// </summary>
internal sealed class WorkerResources
{
    // Future: ObjectPool<BeltSegment> BeltPool
    // Future: ObjectPool<TreeNode> NodePool
    // Future: List<SharedNode> CreatedNodes
