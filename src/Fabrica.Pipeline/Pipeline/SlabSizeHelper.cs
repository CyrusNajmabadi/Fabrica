...
/// the largest power-of-2 element count whose backing array stays below the LOH threshold (~85,000 bytes). Power-of-2 sizing
/// enables bit-shift division and bitwise-AND modulo for O(1) position-to-offset mapping.
/// </summary>
internal static class SlabSizeHelper<TPayload>
{
    ...
    /// <summary>Number of entries per slab — always a power of 2.</summary>
    ...
    {
        var entrySize = Unsafe.SizeOf<PipelineEntry<TPayload>>();
        var maxElements = Math.Max((LargeObjectHeapThreshold - ArrayBaseOverhead) / entrySize, 1);
