...
This plan was fully implemented and benchmarked (PR #162) but **not merged** because it did not show sufficient benefit.

The original ~28x overhead motivating this work turned out to be **false sharing** in the benchmark itself: all jobs wrote to a shared 256-byte `_workArray`, causing cache-line bouncing across 16 cores. After fixing the benchmark to use per-job local arrays, the actual scheduling overhead dropped from ~12ms to ~1.5ms — revealing that the scheduler was already performing well.

### Benchmark results (with false-sharing fix applied at each stage)

| PR | Change | Mean | vs Baseline |
|---|---|---|---|
| #157 | Baseline (benchmark introduced) | 1.703 ms | — |
| #160 | Random steal targets (FastRand) | 1.693 ms | -0.6% |
| #161 | Adaptive worker parking (Tokio-style) | 1.439 ms | -15.5% |
| #162 | Steal-half + zero-alloc queues | 1.465 ms | -14.0% |

Steal-half was roughly neutral vs the adaptive-parking baseline (1.465ms vs 1.439ms), adding complexity without measurable throughput gain at this scale (16 cores, 178 jobs). The adaptive parking from PR #161 was the real win.

The research and implementation details above remain valuable reference material for future work if the job count or core count increases to the point where linear steal-one becomes a bottleneck.
