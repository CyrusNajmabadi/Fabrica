...
/// structural sharing (path-copy), publish (increment roots via the store test accessor), and release
/// (<see cref="GlobalNodeStore{TNode,TNodeOps}.ReleaseSnapshotSlice"/>) in various orders. Verifies
/// refcount correctness at each step.
/// </summary>
public class SnapshotLifecycleTests
{
    ...
    public void FIFO_Release_SequentialSnapshots(int snapshotCount)
    {
        ...
        var roots0 = NonCopyableUnsafeList<Handle<TreeNode>>.Create();
        roots0.Add(root);
        ...
        var lastRoot = slices[snapshotCount - 1].Roots[0];

        // Release FIFO (oldest first)
        for (var i = 0; i < snapshotCount; i++)
        {
            _store.ReleaseSnapshotSlice(slices[i]);
    ...
    public void LIFO_Release_SequentialSnapshots(int snapshotCount)
    {
        ...
        var firstRoot = slices[0].Roots[0];
    ...
    public void Interleaved_CreateAndRelease_SteadyState()
    {
        ...
        var prevRoots = NonCopyableUnsafeList<Handle<TreeNode>>.Create();
        prevRoots.Add(root);
        ...
        {
            var newRoot = this.PathCopyLeftSpine(root, 3);
            var newRoots = NonCopyableUnsafeList<Handle<TreeNode>>.Create();
    ...
    public void MultipleRoots_IndependentTrees_AllFreedOnRelease()
    {
        ...
        _store.ReleaseSnapshotSlice(slice);
    ...
    public void WindowedRelease_ProducerAhead_ConsumerCatchesUp(int windowSize)
    {
        ...
        var initialRoots = NonCopyableUnsafeList<Handle<TreeNode>>.Create();
        ...
        for (var i = 0; i < 30; i++)
        {
            ...
            while (queue.Count > windowSize)
            {
                ...
                _store.ReleaseSnapshotSlice(oldSlice);
            }
        }
    ...
    public void Reroot_InternalNodeBecomesRoot_SurvivesOldSnapshotRelease()
    {
        ...
        // Snapshot 1: root = A
        var snap1Roots = NonCopyableUnsafeList<Handle<TreeNode>>.Create();
        snap1Roots.Add(a);
        ...
        var snap2Roots = NonCopyableUnsafeList<Handle<TreeNode>>.Create();
        snap2Roots.Add(b);
        ...
        _store.ReleaseSnapshotSlice(snap1);
        ...
        _store.ReleaseSnapshotSlice(snap2);
    ...
    public void Reroot_ProgressivelyDeeper_ThreeSnapshots()
    {
        ...
        snap2Roots.Add(b);
        var snap2 = _store.GetTestAccessor().BuildSnapshotSlice(snap2Roots);
        var snap3Roots = NonCopyableUnsafeList<Handle<TreeNode>>.Create();
        snap3Roots.Add(c);
        ...
        _store.ReleaseSnapshotSlice(snap3);
        this.AssertNodeDead(c);
    }
    ...
    public void FourSnapshots_AllReleasePermutations()
    {
        ...
        foreach (var perm in permutations)
        {
            ...
            var newestRoot = slices[3].Roots[0];

            foreach (var idx in perm)
                store.ReleaseSnapshotSlice(slices[idx]);

            Assert.Equal(0, store.RefCounts.GetCount(newestRoot));
        }
    }
